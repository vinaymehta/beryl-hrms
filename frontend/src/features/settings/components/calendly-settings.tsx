 ...
 *
 * Moved out of the page and into the Settings workspace — the queries,
 * mutations and the OAuth-return handling below are unchanged from when this
 * lived at /settings/interviews, which is still the route the backend
 * redirects to (with ?connected=true or ?error=...).
 */
export function CalendlySettings() {
  ...
  useEffect(() => {
    if (searchParams.get("connected") === "true") {
      ...
      window.history.replaceState(null, "", "/settings/interviews")
  ...
  const connect = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof Error ? err.message : "Couldn't start the Calendly connection"),
  })
  ...
  const setEventType = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof Error ? err.message : "Couldn't save the event type"),
  })
  ...
  const registerWebhook = useMutation({
    ...
    onError: (err) =>
      toast.error(err instanceof Error ? err.message : "Couldn't register the webhook"),
  })
  ...
  const activeEventType = eventTypes?.find(
    (t) => t.uri === (selectedEventType || connection?.defaultEventTypeUri)
  )
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Shortlisted candidates are emailed a link to book their own interview slot. Calendly owns
          the availability — set your working-day slots on the event type you choose below.
        </CardDescription>
      </CardHeader>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 rounded-xl border border-dashed p-8 text-center">
            ...
            >
              ...
              {/* Names what the button DOES — it starts the OAuth connect
                    flow. "Manage Calendly connection" is the nav item that
                    brings you to this section, not this button. */}
              Connect Calendly
            </Button>
          </div>
        ) : (
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <SelectTrigger aria-label="Interview event type" className="flex-1">
                    <SelectValue
                      placeholder={typesLoading ? "Loading event types…" : "Choose an event type"}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {(eventTypes ?? []).map((t) => (
                ...
                <Button
                  ...
                  disabled={
                    !selectedEventType ||
                    selectedEventType === connection.defaultEventTypeUri ||
                    setEventType.isPending
                  }
                >
                  {setEventType.isPending ? (
                ...
                <div className="min-w-0 text-xs">
                  <p className="font-medium text-foreground">
                    {connection.webhookRegistered
                      ...
                      : "Booking notifications not set up"}
                  </p>
